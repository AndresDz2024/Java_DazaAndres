...
public class Factura {
    ...
    private List<DetalleFactura> detalles; // Lista para manejar los productos y sus cantidades
    ...
    public void agregarProducto(Producto producto, int cantidad) {
        ...
        total += producto.getPrecio() * cantidad; // Actualizar el total

        // Insertar el detalle de la factura en la base de datos
...
public void calcularTotalConDescuento(Function<Double, Double> descuento) {
    this.totalConDescuento = descuento.apply(this.total); // Calcular el total con el descuento
}
    ...
    public void guardarFactura() {
        ...
        try (Connection conn = new Conexion().establecerConexion();
            ...
            stmt.setDouble(2, total); // Guardar el total original
            stmt.setDouble(3, totalConDescuento); // Guardar el total con descuento
            stmt.executeUpdate();
        ...
        try (Connection conn = new Conexion().establecerConexion();
            ...
            if (rs.next()) {
                ...
                if (cliente != null) {
                    ...
                    factura.id = id;  // Asignar el ID de la factura
                    return factura;
                }
            }
        } catch (SQLException e) {
    ...
    public static void listarFacturas() {
        // Implementación para listar facturas
